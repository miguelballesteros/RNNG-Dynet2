...
struct ParserBuilder {
  ...
  //Parameter* p_pbias2; // parser state bias
  //Parameter* p_A2; // action lstm to parser state
  //Parameter* p_S2; // stack lstm to parser state
...
int main(int argc, char** argv) {
  ...
  if (conf.count("train")) {
    ...
    while(!requested_stop) {
      ...
      if (logc > 50) {
        ...
        double x;
	vector<unsigned> pred;
        parser.log_prob_parser(&cg, pred, parser::Sentence(), vector<int>(),&x,true);
      }
  ...
  if (test_corpus.size() > 0) {
	...
	vector<unsigned> pred;
        Expression loss=parser.log_prob_parser(&hg,pred,sentence,actions,&right,true);
        lp = as_scalar(hg.incremental_forward(loss));
      }
